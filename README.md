...
Every ranking decision traces to a 71-query labeled benchmark (P@k, MRR, nDCG@10, Recall@100) with keyword / paraphrase / technique slices — it's why BM25 stays the default and the fancier rankers are opt-in. The numbers and their write-ups live in [experiments/](experiments/).
...
- **Stemming.** Would fix `tree`/`trees` and `merge`/`merging` in general rather than one alias at a time, but it rewrites every document and every query and invalidates the committed embeddings. Nine aliases covered every name reported so far, so the blast radius isn't justified yet.
