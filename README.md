...
To run it: `docker compose up -d`, copy `.env.example` to `.env`, `npm install`, `npm run db:migrate`, `npm run dev`, open `http://localhost:3000`. Implementation notes that go deeper live in `docs/implementation/`.
