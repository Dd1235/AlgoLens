...
- **It tells you what exists.** Search `dp` and the line above the results names the family — `state-compression`, `bitmask-dp`, `tree-dp`, `digit-dp`, `slope-trick` — each one clickable to browse every problem carrying it. The point is the vocabulary: you can't search for a technique you've never heard of, and most people's exposure stops at one sheet.
