...
Then hit BM25's wall. Ask "check if brackets open and close in the right order" and it returns problems about opening water taps — the corpus says "parentheses", the query says "brackets", and no tf·idf math bridges that. So: dense retrieval. Every problem gets embedded offline with a quantized MiniLM sentence-transformer (384-dim ONNX running in-process via transformers.js — no Python, no API keys), and the 1198 unit vectors are a 1.75 MB file committed to the repo. At query time the only new work is embedding the query (~0.5 ms on CPU); scoring is a brute-force dot product over one contiguous Float32Array — 0.8 ms for the whole corpus, which is the measured argument for why this project does not need a vector database. A `hybrid` ranker fuses BM25 and dense top-100s with reciprocal rank fusion, same `search(q, k, offset)` seam, fourth and fifth entries in the ranker registry.
