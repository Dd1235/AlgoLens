...
What I learned: the ranker abstraction is the whole point — once `SearchIndex` was in place, swapping in a polyglot service over gRPC was mechanical. Stopword lists are corpus-specific and the defaults will burn you. BM25 isn't strictly better than TF-IDF on every query, but on heterogeneous-length corpora it's the safer default by a margin you can measure. Pagination's `total` belongs after filtering, not before, or "load more" lies. Postgres for user state, JSON corpus for problems — keep the search index pure and let user state be a post-filter. And single-service Node is plenty for 1,185 docs; the Go microservice is a benchmark artifact, not a load-bearing piece of prod.
